...
const PLANS = [
  ...
  },
  {
    ...
    features: [
      "Todo lo anterior",
      "Métricas avanzadas: VO2Max, lactato estimado",
      "Planes por ritmo (pace) 100% personalizados",
      "Análisis biomecánico de tu técnica por IA",
      "Sesiones 1:1 con PaceAI coach sin límites",
      "Ajuste semanal del plan según tus sensaciones",
    ],
...
Hora: ${hora}`);
  ...
  const genTrainPlan = async (race, currentUser = user) => {
    ...
    // Calculamos en DÍAS completos y luego convertimos a semanas necesarias
    // para que el día de la carrera quede DENTRO del rango generado.
    const totalDays = Math.round((raceDate - planCreation) / (24 * 60 * 60 * 1000));
    const weeksAvailable = Math.max(4, Math.floor(totalDays / 7) + 1);
    ...
    try {
      ...
      // ── FIX: forzar la sesión de carrera al ÍNDICE correcto dentro de la última semana ──
      // No confiamos en que la IA pusiera tipo:"Carrera" en el índice correcto.
      // Calculamos el offset real en días desde planCreation hasta race.date,
      // y de ahí derivamos en qué posición (0-6) de la última semana debe caer.
      if (plan?.semanas?.length) {
        const lastWeek = plan.semanas[plan.semanas.length - 1];
        if (lastWeek?.sesiones?.length === 7) {
          const totalDaysToRace = Math.round((raceDate - planCreation) / (24 * 60 * 60 * 1000));
          const correctIdx = totalDaysToRace % 7; // posición dentro de su semana
        ...
        {sem.sesiones && (
          <div className="wcont">
            ...
            {sem.sesiones.map((s, i) => {
              const planStart = planStartDate || new Date();
              // Offset directo: día 0 del plan es planStart, avanzamos día a día.
              // Forzamos mediodía para evitar bugs de zona horaria al sumar días.
              const dayOffset = activeWeek * 7 + i;
              const realDate = new Date(
                planStart.getFullYear(),
                planStart.getMonth(),
                planStart.getDate() + dayOffset,
                12, 0, 0
              );
  ...
  const renderZones = () => {
    ...
    // Inicializa Leaflet dinámicamente
    ...
    useEffect(() => {
      ...
      const initMap = async () => {
        ...
        // Inicializar mapa o reusar existente
        ...
        // Polilínea roja del circuito
        if (activeZone.route?.length > 1) {
          L.polyline(activeZone.route, {
            color: "#FF4500",
            weight: 5,
            opacity: 0.85,
            lineJoin: "round",
          ...
          L.circleMarker(activeZone.route[0], {
            radius: 8,
            fillColor: "#22c55e",
            color: "white",
            weight: 2,
        ...
        // Centrar y hacer zoom al circuito
        if (activeZone.route?.length > 1) {
          const bounds = L.latLngBounds(activeZone.route);
    ...
    return (
      <div className="pw">
        ...
        <p style={{ color: "var(--mu)", fontSize: ".88rem", marginBottom: 20 }}>
          Circuitos de running en Buenos Aires. El trazado rojo muestra el recorrido aproximado de cada zona.
        </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "340px 1fr", gap: 18 }}>
              ...
              return (
                <div key={zone.id} onClick={() => setSelectedZone(zone)}
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    <span style={{ fontSize: ".7rem", padding: "2px 8px", borderRadius: 20, background: diffColor[zone.difficulty] + "22", color: diffColor[zone.difficulty], fontWeight: 700 }}>{zone.difficulty}</span>
          ...
          <div>
            ...
            <div style={{ borderRadius: 12, overflow: "hidden", border: "1px solid var(--bd)", height: 340, marginBottom: 14, position: "relative" }}>
              ...
              {/* Leyenda */}
              <div style={{ position: "absolute", bottom: 10, left: 10, zIndex: 1000, background: "rgba(0,0,0,.75)", borderRadius: 8, padding: "6px 12px", display: "flex", gap: 14, fontSize: ".72rem" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  <span style={{ display: "inline-block", width: 16, height: 3, background: "#FF4500", borderRadius: 2 }} />
                  Circuito
                </span>
                <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
